...
export default function AddExercise({ GlobalState }) {
  const { readFromFirebase, setReadFromFirebase } = GlobalState;
    ...
    if (exerciseToAdd && exerciseToAdd.length > 0) {
      ...
      await addDoc(dbRef, data)
        .then(() => {
          ...
          setReadFromFirebase(true);
          console.log("Read from fb?" + readFromFirebase);
        })
        .catch((error) => {
          console.log(error);
        });
